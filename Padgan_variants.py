...
def train_padgan(X, N, Y, config_params=None, train_params=None, DTAI_params=None, classifier_params=None, regressor_params=None):
    scaling, DTAI, CLF, classifier, regressor= config_params
    ...
    model = PadGAN(lambda0=lambda1, lambda1=lambda1, noise_dim = noise_dim, data_dim = len(X[0,:]))
    def pad_aux_loss_wrapper(X):
        return pad_aux_loss(X, regressor, classifier, y_scaler, eval_func)
    model.train(X, pad_aux_loss_wrapper, steps=steps)
    return model
    
def padgan_wrapper(config_params=None, train_params=None, DTAI_params=None, classifier_params=None, regressor_params=None):
    def model(X, N, Y):
        return train_padgan(X, N, Y, config_params=config_params, train_params=train_params, DTAI_params=DTAI_params, classifier_params=classifier_params, regressor_params=regressor_params)
    return model
